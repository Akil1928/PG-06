...
public class AdjacencyMatrixGraph<T extends Comparable<T>> implements Graph<T> {
    ...
    public void removeVertex(T element) throws GraphException, ListException {
        ...
        if(index!=-1){ //si el vértice existe en la lista de vértices
            ...
            // Limpiamos la última fila y columna (la posición counter-1)
            for (int i = 0; i < counter; i++) {
                //cambia las filas y la col es la última (columna "sucia")
                adjacencyMatrix[i][counter-1] = (T) Integer.valueOf(0);
                //cambia las cols y la fila es la última (fila "sucia")
                adjacencyMatrix[counter-1][i] = (T) Integer.valueOf(0);
            }
            // Limpiamos el último vértice
            vertexList[counter-1] = null;

            //al final decrementamos el contador de vértices existente
            counter--;
        }
    }
